...
/**
 * The in-app notification inbox (SCRUM-194 seller, SCRUM-196 buyer).
 *
 * One implementation for both roles: the categories are role-agnostic on the
 * server, and a buyer's Documents tab is the same query as a seller's. Only
 * the route it links back to and the subtitle differ, so those are props
 * rather than a second copy of two hundred lines.
 *
 * Server Component on purpose. The tabs are links and the search box is a GET
 * form, so a filtered view is a real URL — shareable, reloadable, and back-
 * button-correct — instead of client state that evaporates on refresh.
 */
export async function NotificationInbox({
  ...
  token: string;
  /** Where the tabs and search form point — the caller's own route. */
  basePath: string;
  subtitle: string;
  category?: string;
  q?: string;
}) {
  const tab = parseTab(category);
  const q = rawQ?.trim() ?? '';
