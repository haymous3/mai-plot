"""Authentication primitives for protected listing endpoints.

`get_current_user` (wired in dependencies.py) validates the bearer access
token and yields a CurrentUser. A failure raises AuthenticationError, which
main.py maps to a 401 in the standard error envelope. Mirrors auth-service's
security module so token handling is consistent across services.
"""
