...
Every Redis read in this service goes through `get_with_fallback`. If Redis
raises — connection refused, timeout, cluster down — the helper swallows the
error, logs it, and calls the fallback function. Redis failure must never
crash a request. Ported verbatim from auth-service so cache semantics are
...
async def get_with_fallback[T](
    ...
    *,
    ...
    `redis` may be None to short-circuit caching entirely — useful in tests
    and in code paths where the value should always be re-derived.
    """
