"""Celery queue routing (listing-service, SCRUM-210).
...
from app.services.expiry_notifier import CeleryExpiryNotifier
...
def test_this_services_tasks_run_on_its_own_queue() -> None:
    ...
    assert celery_app.conf.task_default_queue == "listing-service"


@pytest.mark.asyncio
async def test_expiry_warning_goes_to_the_notification_queue() -> None:
    notifier = CeleryExpiryNotifier(broker_url=_BROKER)
    recorder = _RecordingApp()
    notifier._app = recorder

    await notifier.expiry_warning(seller_id=uuid4(), listing_id=uuid4())
