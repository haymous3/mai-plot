"""Authentication primitives for protected document endpoints.

Mirrors auth-service / listing-service: get_current_user validates the bearer
access token, AuthenticationError maps to a 401 envelope in main.py.
"""

from __future__ import annotations

from dataclasses import dataclass
from uuid import UUID


class AuthenticationError(RuntimeError):
    """Raised when a request lacks a valid bearer access token."""

    def __init__(
        self,
        code: str = "UNAUTHORIZED",
        message: str = "Authentication required.",
    ) -> None:
        self.code = code
        self.message = message
        super().__init__(message)


class AdminAccessError(RuntimeError):
    """Raised when an authenticated caller fails the admin gate (wrong role or
    a non-whitelisted IP). Maps to 403, not 401 — authenticated, not authorised."""

    def __init__(self, code: str, message: str) -> None:
        self.code = code
        self.message = message
        super().__init__(message)


@dataclass(frozen=True)
class CurrentUser:
    user_id: UUID
    role: str


def parse_bearer(authorization: str | None) -> str:
    """Extract the token from an `Authorization: Bearer <token>` header."""
    if not authorization:
        raise AuthenticationError(message="Missing Authorization header.")
    parts = authorization.split(" ", 1)
    if len(parts) != 2 or parts[0].lower() != "bearer" or not parts[1].strip():
        raise AuthenticationError(message="Authorization header must be 'Bearer <token>'.")
    return parts[1].strip()
