...
The audit_log table is owned by analytics-service (append-only triggers).
document-service only INSERTs (document verify/reject), via raw SQL — it has
no ORM models, and the JSONB columns are cast explicitly. INSERT-only.
"""
