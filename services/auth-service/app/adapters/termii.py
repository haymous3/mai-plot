"""Termii adapter — SMS dispatch for OTP messages.
...
Production binds the real one via TERMII_USE_FAKE=false. CI and local
dev get the fake by default, matching .env.example.
"""
...
class HttpTermiiClient:
    ...
    async def send_sms(self, phone: str, message: str) -> None:
        # Termii's `to` field expects local format without the leading +.
        payload = {
            "to": phone.lstrip("+"),
