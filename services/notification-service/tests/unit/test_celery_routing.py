"""Celery queue routing (notification-service, SCRUM-210).
...
def test_this_services_tasks_run_on_its_own_queue() -> None:
    ...
    assert celery_app.conf.task_default_queue == "notification-service"


def test_the_fan_out_tasks_inherit_this_queue() -> None:
    """dispatch fans out with `.delay()` on tasks owned by THIS app, so they take
    task_default_queue automatically — which is what stops the email task being
    grabbed by realtor-celery-worker, as the Railway logs caught it doing."""
    from app.tasks.email import send_email_notification

    assert send_email_notification.app.conf.task_default_queue == "notification-service"
